...
BEGIN
  l_plans_loaded := DBMS_SPM.load_plans_from_cursor_cache(
    sql_id => '4v8j2878sfv75');
END;


-------------------------Evolving SQL Plan Baselines ----- for accepting plan
...
SET LONG 10000
SELECT DBMS_SPM.evolve_sql_plan_baseline(sql_handle => 'SYS_SQL_7b76323ad90440b9', plan_name => '')
FROM   dual;



/*load sql with other plan*/  ---load old sql_id plan  to new sql_handle plan ---- sql_id => sql_id of desired plan ----- plan_hash_value of desired plan---sql_handle = > broken sql_handle
DECLARE
  l_plans_loaded  PLS_INTEGER;
BEGIN
  l_plans_loaded := DBMS_SPM.load_plans_from_cursor_cache( sql_id => 'dg8y9nu01hxww',plan_hash_value=>3379702144,SQL_HANDLE=>'SQL_0389fd1927e28724') ;
END;


/*load sql with other plan*/  ---load old sql_id plan  to new sql_handle plan ---- sql_id => sql_id of desired plan         sql_handle = > broken sql_handle
DECLARE
  l_plans_loaded  PLS_INTEGER;
BEGIN
  l_plans_loaded := DBMS_SPM.load_plans_from_cursor_cache( sql_id => 'cc85c666v3yux',SQL_HANDLE=>'SQL_21cc17b1dc522697') ;
END;
...
-- The DROP_SQL_PLAN_BASELINE function can drop a specific plan from a baseline, or all plans if the plan name is not specified.
    ...
    BEGIN
      l_plans_dropped := DBMS_SPM.drop_sql_plan_baseline (
        sql_handle => NULL,
        plan_name  => 'SQL_PLAN_br48wbk74n4ba4367405b');
...
BEGIN
  l_plans_altered := DBMS_SPM.alter_sql_plan_baseline(
    sql_handle      => 'SQL_162e41f147ee4f32',
    plan_name       => 'SQL_PLAN_072gx34my51t487cb976f',
...
SELECT * FROM   TABLE(DBMS_XPLAN.display_sql_plan_baseline(plan_name=>'SQL_PLAN_072gx34my51t487cb976f'));
